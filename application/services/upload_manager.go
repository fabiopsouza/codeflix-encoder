...
func (vu *VideoUpload) UploadObject(objectPath string, client *storage.Client, ctx context.Context) error {
	...
	//wc.ACL = []storage.ACLRule{{Entity: storage.AllUsers, Role: storage.RoleReader}} ACL is deprecated and throw error
...
func (vu *VideoUpload) uploadWorker(in chan int, returnChan chan string, uploadClient *storage.Client, ctx context.Context) {
	...
	for pathIndex := range in {
		err := vu.UploadObject(vu.Paths[pathIndex], uploadClient, ctx)

		if err != nil {
			vu.Errors = append(vu.Errors, vu.Paths[pathIndex])
			log.Printf("error during the upload: %v. Error: %v", vu.Paths[pathIndex], err)
